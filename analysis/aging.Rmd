...
gtbl <- dplyr::inner_join(as.data.frame(org.Hs.egGENENAME),as.data.frame(org.Hs.egENSEMBL)) %>%
...
standard_plot_this <- dplyr::bind_rows(query,subject) %>% dplyr::distinct()

# We can't simply divide "same" by 2 here because some genes have multiple reciprocal hits
# which means they are sometimes assigned as offspring or parent and same at the same time.
# We need to identify those genes and count them as two events rather than one.
#
# standard_normalized_events <- standard_plot_this %>% 
#   dplyr::group_by(gene_id) %>% 
#   dplyr::group_by(branch,relation) %>%
#   dplyr::summarize(`Number of genes`=n()) %>%
#   dplyr::ungroup() %>%
